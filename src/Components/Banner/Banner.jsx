...
const Banner = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-6 relative">
      <div className="slider-container w-full h-auto overflow-visible">
        <Slider {...settings}>
          {products?.map((product) => (
            <div
              key={product?.id}
              className="banner_slide_item flex flex-col sm:flex-row items-center justify-between w-full"
            >
              <div className="banner_text max-w-full sm:max-w-[60%] md:max-w-[50%] p-4 sm:p-6 text-center sm:text-left">
                <p className="text-lg sm:text-xl md:text-2xl font-inter text-[#272343] uppercase font-normal">
                  {product?.subTitle}
                </p>
                <h3 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-[#272343] font-inter capitalize leading-tight sm:leading-normal md:leading-12 lg:leading-16 max-w-[631px] w-full font-bold mb-4 sm:mb-5">
                  {product?.title}
                </h3>
                <button
      ...
      >
        <img
          src="public/assets/download.jpg" // Replace with your WhatsApp icon path
          alt="WhatsApp"
